...
ファームウェア更新でNintendo Switchにも対応しています。  
その場合は下記コントローラ互換品として動作します。  
...
| トリガボタンＡ'、Ｂ' | ボタンＡ、Ｂ |  |
| セレクト、スタートボタン | ボタンＨＯＭＥ、ＳＴＡＲＴ | CZ-8NJ2のデジタルモードでは使用できません。 |
...
1. デバイスを一度取り外します。
2. コントローラの［Cボタン］を押します。（コントローラモードスイッチはどちらでも可）
3. Windows PCに接続します。
4. デバイス接続音がしたら、コントローラのボタンを放してください。
5. Firmware Updateソフトを実行します。
6. 正常に認識されると自動的にファームウェア更新が実行されます。
7. 更新後、正常に動作しない場合はBボタンを押しながらUSBケーブルを抜き差しを行いE2PROMを初期化してください。
  

### Firmware Updateソフトについて

下記URLにて頒布しています。

メガドライブ2ミニ向け　　　　<./FFU/update_mdmini.bat>  
Nintendo Switch向けFirmware　　<./FFU/update_switch.bat>  
