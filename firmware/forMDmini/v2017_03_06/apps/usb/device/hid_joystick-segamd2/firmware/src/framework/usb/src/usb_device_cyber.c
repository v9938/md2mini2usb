...
#if !defined(USB_USER_DEVICE2_DESCRIPTOR)
    //Device descriptor
    extern const USB_DEVICE_DESCRIPTOR device_dsc2;
#else
    USB_USER_DEVICE2_DESCRIPTOR_INCLUDE;
#endif
        ...
        switch(SetupPkt.bDescriptorType)
        {
            case USB_DESCRIPTOR_DEVICE:
                	...
                	if (MDMODE_EQON){
	                    ...
	                    inPipes[0].pSrc.bRom = (const uint8_t*)&device_dsc2;
		                inPipes[0].wCount.Val = sizeof(device_dsc2);
		            }
                #else
                	if (MDMODE_EQON){
                        ...
                        inPipes[0].pSrc.bRom = (const uint8_t*)USB_USER_DEVICE2_DESCRIPTOR;
    	                inPipes[0].wCount.Val = sizeof(device_dsc);
		            }
                #endif
                break;
            ...
            case USB_DESCRIPTOR_STRING:
                ...
                if(SetupPkt.bDscIndex<USB_NUM_STRING_DESCRIPTORS -1)
                {
                	...
                	if (MDMODE_EQON)
	                    ...
	                    inPipes[0].pSrc.bRom = *(USB_SD2_Ptr+SetupPkt.bDscIndex);

                    // Set data count
                    inPipes[0].wCount.Val = *inPipes[0].pSrc.bRom;                    
                }else if (SetupPkt.bDscIndex == USB_NUM_STRING_DESCRIPTORS -1 )
                 {
                    inPipes[0].info.bits.ctrl_trf_mem = USB_EP0_RAM;    // Set memory type
                    inPipes[0].pSrc.bRam = (uint8_t*)SN_update;          // Set Source
                    inPipes[0].wCount.v[0] = SN_update[0];            // Set data count        
                    inPipes[0].info.bits.busy = 1;
                 }
